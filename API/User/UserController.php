    ...
    private function addUser()
    {
        if (!empty($this->params['data'])) {
            ...
            if ($valid) {
                ...
                $sql = "INSERT INTO users (user_id,user_firstname,user_lastname,user_username,user_email,user_password) values(?, ?, ?, ?, ?, ?)";
                $q = $pdo->prepare($sql);
                $q->execute(array($user_id,$user_firstname,$user_lastname,$user_username,$user_email,md5($user_password)));
                Database::disconnect();
